...
export const useTabsViewStore = defineStore({
  ...
  actions: {
    ...
    closeCurrentTabByPath(fullPath: string) {
      const index = this.tabsList.findIndex((item) => item.fullPath == fullPath);
      if (index < 0)
        return false;
      this.delCompFromClosedTabs(this.tabsList.splice(index, 1));
    },
    addTabByPath(fullPath: string,params:any) {
      this.closeCurrentTabByPath(fullPath);

      router.push({name: fullPath,state:{params:params}});
    },
    openTab(fullPath: string,params:any) {
      router.push({name: fullPath,state:{params:params}});
    },
  },
});
